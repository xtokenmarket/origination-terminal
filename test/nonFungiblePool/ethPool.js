...
const { increaseTime, bn, getBalance } = require("../../scripts/helpers");

describe("NFT sale with ETH as payment", async () => {
  beforeEach(async () => {
    ({ accounts, purchaseToken, originationPoolETH, listedNftETHPool } = await createFixture());
  ...
  it("should receive eth back if accidentally sent more", async () => {
    ...
    await expect(await originationPoolETH.connect(user).publicMint(3, { value: cost.mul(2) })).to.changeEtherBalance(user, cost.mul(-1));
  });
  ...
  it("shouldn't be able to mint with less eth than required", async () => {
    ...
    let price = await originationPoolETH.getCurrentMintPrice();
    await expect(originationPoolETH.connect(user).publicMint(1, { value: price.sub(1) })).to.be.revertedWith("Insufficient payment");
  });

  it("shouldn't be able to mint nft before sale start", async () => {
    await expect(originationPoolETH.connect(user).publicMint(1)).to.be.revertedWith("Not public mint period");
  });
  ...
  it("shouldn't be able to mint more than mint cap", async () => {
    ...
    await expect(originationPoolETH.connect(user).publicMint(1001)).to.be.revertedWith("Total mint cap reached");
  });
  ...
  it("owner should be able to retrieve invested tokens after sale end", async () => {
    ...
    await expect(await originationPoolETH.claimPurchaseTokens()).to.changeEtherBalance(deployer, claimAmount);
  });
