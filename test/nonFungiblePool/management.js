...
describe("NFT sale management functions", async () => {
  beforeEach(async () => {
    ({ accounts, purchaseToken, originationPool, listedNft, whitelistRoot } = await createFixture());
  ...
  it("shouldn't be able to set a whitelist after sale has started", async () => {
    ...
    await expect(originationPool.setWhitelist(whitelistRoot)).to.be.revertedWith("Cannot set whitelist after sale initiated");
  });
